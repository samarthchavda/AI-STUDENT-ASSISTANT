...
export async function completeDailyChallenge(
  ...
  const response = await api.post(`/daily-challenge/complete/${challengeId}`, {
...
export async function getChallengeHistory(limit: number = 30) {
  const response = await api.get('/daily-challenge/history', {
    params: { limit }
  });
  return response.data;
}
...
export async function getCompanySheets(): Promise<CompanySheet[]> {
  const response = await api.get('/company-sheets/list');
  return response.data;
}

export async function getSheetQuestions(sheetId: number): Promise<CompanySheetQuestion[]> {
  const response = await api.get(`/company-sheets/${sheetId}/questions`);
  return response.data;
}
...
export async function markQuestionComplete(
  ...
  const response = await api.post(
    `/company-sheets/${sheetId}/questions/${questionSlug}/complete`
  );
  return response.data;
}
...
export async function checkAIAccess(): Promise<{
  ...
  const response = await api.post('/subscription/check-ai-access');
  return response.data;
}

export async function recordAIUsage(): Promise<{ message: string }> {
  const response = await api.post('/subscription/use-ai');
  return response.data;
}
...
export async function upgradeToPremium(
  ...
  const response = await api.post('/subscription/upgrade-to-premium', {
