...
export const DarkTechSidebarProPrint: React.FC<TemplateProps> = ({ data, themeColor = '#06b6d4' }) => (
  <div className="bg-white w-full font-sans" style={{ display: 'table', minHeight: '297mm' }}>
    <div style={{ display: 'table-row' }}>
      {/* Left Sidebar */}
      <div style={{ display: 'table-cell', width: '40%', backgroundColor: '#000000', color: '#ffffff', padding: '48px 32px', verticalAlign: 'top' }}>
        <div style={{ marginBottom: '40px' }}>
          <h1 style={{ fontSize: '26px', fontWeight: '700', marginBottom: '8px', color: themeColor }}>
            {data.identity.fullName}
          </h1>
          <p style={{ fontSize: '12px', color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
        ...
        <section style={{ marginBottom: '40px' }}>
          <h2 style={{ fontSize: '11px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '16px', paddingBottom: '8px', borderBottom: `1px solid ${themeColor}`, color: themeColor }}>
            CONTACT
          </h2>
          <div style={{ fontSize: '11px', color: '#d1d5db', lineHeight: '1.8' }}>
            <div style={{ wordBreak: 'break-all' }}>{data.identity.email}</div>
          ...
          <section style={{ marginBottom: '40px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {data.skills.map((skillGroup, idx) => (
                <div key={idx}>
                  <h3 style={{ fontSize: '11px', fontWeight: '600', marginBottom: '8px', color: themeColor }}>
                    {skillGroup.category}
                  </h3>
          ...
          <section style={{ marginBottom: '40px' }}>
            ...
            {data.education.map((edu) => (
              <div key={edu.id} style={{ marginBottom: '16px' }}>
                <h3 style={{ fontWeight: '700', fontSize: '12px', color: '#e5e7eb' }}>{edu.degree}</h3>
                {edu.field && <p style={{ fontSize: '11px', color: '#9ca3af' }}>{edu.field}</p>}
                <p style={{ fontSize: '11px', color: '#9ca3af', marginTop: '4px' }}>{edu.school}</p>
                <p style={{ fontSize: '10px', color: '#6b7280' }}>{edu.startDate} - {edu.endDate}</p>
              </div>
            ))}
          </section>
        )}
      ...
      <div style={{ display: 'table-cell', padding: '48px', verticalAlign: 'top', color: '#111827' }}>
        {/* Summary */}
        <section style={{ marginBottom: '40px' }}>
          <h2 style={{ fontSize: '13px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '16px', paddingBottom: '8px', borderBottom: `2px solid ${themeColor}`, color: themeColor }}>
